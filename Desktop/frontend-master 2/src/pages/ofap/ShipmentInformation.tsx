...
const ShipmentInformation = (props: any) => {
    ...
    const [includeHaulage, setIncludeHaulage] = useState(false);
    ...
    const contactInfo = {
        "NG": <>A mail has been sent to you detailing the next steps and our sales agent will contact you in 1-2 business days. If you have any further questions please call Kayode on <a className='green-text' href='tel:+234906 688 5913'>+234906 688 5913</a></>,
        "KE": <>A mail has been sent to you detailing the next steps and our sales agent will contact you in 1-2 business days. If you have any further questions please call Kenneth on <a className='green-text' href='tel:+234906 688 5913'>+254 733 437 698</a></>
    }

    useEffect(() => {
        if (booking_summary?.ocean_freight?._id !== undefined) {
            const charges = booking_summary?.ocean_freight?.charges[0];
            const agencyCharge = charges?.filter(item => item.rateBasis === "PER_JOB")[0]?.amount;

            const chargesObject = {};

            booking_summary?.ocean_freight?.rates_data?.offer_type === "SPOT" ?  
            charges.forEach(charge => {
                chargesObject[charge.description] = charge.rateBasis.toLowerCase() === 'per_job'
                ? charge.amountUsd
                : charge.amountUsd * counter;
            }) :
            charges.forEach(charge => {
            ...
            booking_summary?.ocean_freight?.rates_data?.offer_type === "SPOT" 
            ? setTotalAmount((booking_summary?.ocean_freight?.rates_data.total_amount_usd * counter) - ((counter - 1) * agencyCharge)) 
                ...
                {loading || getting_booking ? <PageLoading title="shipment information" /> :
                    <>
                        <div className="w-full lg:w-4/5">
                                        ...
                                        <div className="hidden md:block bg-purples-100 col-span-2 md:pl-4 border-t-[1px] border-solid border-[#E5E7EB] md:border-t-[0px] pt-6 md:pt-0">
                                            ...
                                            <div className="grid grid-cols-2 gap-x-4 md:gap-x-0 gap-y-4 md:gap-y-0 md:grid-cols-3 md:pt-6 border-solid border-[#E5E7EB] border-b-[1px] md:border-b-[0px] pb-6 md:pb-0">
                                                ...
                                                <SingleSummaryInfo
                                                    text="Free Days"
                                                    value={
                                                        (booking_summary?.ocean_freight?.rates_data?.detention_days || booking_summary?.ocean_freight?.rates_data?.demurrage_days)
                                                         ? booking_summary?.ocean_freight?.rates_data?.detention_days === "N/A" ? 0 + booking_summary?.ocean_freight?.rates_data?.demurrage_days + " days"
                                                         : booking_summary?.ocean_freight?.rates_data?.demurrage_days === "N/A" ? 0 + booking_summary?.ocean_freight?.rates_data?.detention_days + " days" 
                                                         :  booking_summary?.ocean_freight?.rates_data?.demurrage_days + booking_summary?.ocean_freight?.rates_data?.detention_days + " days"
                                                         : "N/A"
                                                        }
                                                />
                                            </div>
                            ...
                            <div className="pb-8">
                                <div className="px-6 py-6 breakdown-border rounded">
                                    ...
                                    <p className="grey-text-4 text-sm font-light bottom-divider-2 pb-4">Payment Breakdown</p>
                                        ...
                                        <div className='flex flex-col gap-y-6'>
                                            {Object.entries(prices).map(([description, amountUsd], idx) => (
                                                <div className="flex items-center justify-between" key={idx}>
                                                    <p className="black-text-3 font-light">{description}</p>
                                                    <p className="black-text-3 font-light">{formatCurrency((amountUsd), "USD")}</p>
                            ...
                            <form onSubmit={handleSubmit(confirmSubmit)}>
                                ...
                                <div className="text-xs grey-text mt-3"><span className='text-red-500'>*</span>{" "}By clicking on Confirm Booking, you indicate your agreement to the Oneport 365 booking <a href="https://www.oneport365.com/terms-of-service" target='_blank' rel="noreferrer" className="green-text">terms and conditions</a>. If you would like to inquire further or speak with a sales representative, click on the Enquiry button.</div>
                        ...
                        <SuccessModal
                            ...
                            subtext={contactInfo[country] || contactInfo["NG"]}
...
export default connect(mapStateToProps, { getBookingSummary, getBookingDetailsById, confirmBooking, clearBooking })(ShipmentInformation);