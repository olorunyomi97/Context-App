...
import ContainerDetails from "./containerTab/containerDetailsTab";
import ContainerStatusHistory from "./containerStatusTab/containerStatusHistoryTab";
import DocumentUpload from './documentTab/documentsDetailsTab';
import DocumentsView from './documentViewTab/documentView'
import ShipmentDetailTab from './shipmentTab/shipmentDetailTab'
import "../index.css";
...
const ShipmentDetails = (props: any) => {
	...
	console.log(single_shipment_data?.shipment_data);
	...
	//   const [tab1, setTab1] = useState("Shipment Details");

	//   // store the tab state in the local storage on component mount
	//   useEffect(() => {
	//     setTab(tab ? tab : "Shipment Details");
	//   }, []);
									...
									<div className="lg:px-7 py-4">
											...
											<div className="grid grid-cols-3 items-center p-5 bg-green lg:rounded-lg">
												...
												<div className="text-center">
													...
													<div className="flex items-center justify-center mb-3">
														<div className="flex items-center">
															<p className="navigation-text">
																...
																{
																	single_shipment_data?.shipment_data?.shipment_type === "export" && single_shipment_data?.shipment_data?.shipment_transport_type === "ocean_freight" ? (
																		single_shipment_data?.shipment_data?.origin_port_code

																	) : single_shipment_data?.shipment_data?.shipment_type === "import" && single_shipment_data?.shipment_data?.shipment_transport_type === "ocean_freight" ? (
																		single_shipment_data?.shipment_data?.port_of_discharge

																	) : single_shipment_data?.shipment_data?.shipment_type === "export" && single_shipment_data?.shipment_data?.shipment_transport_type === "haulage" ? (
																		single_shipment_data?.shipment_data?.stuffing_location

																	) : single_shipment_data?.shipment_data?.shipment_type === "import" && single_shipment_data?.shipment_data?.shipment_transport_type === "haulage" ? (
														...
														<div className="flex items-center ">
															<p className="navigation-text">
																...
																{single_shipment_data?.shipment_data?.shipment_type === "export" && single_shipment_data?.shipment_data?.shipment_transport_type === "ocean_freight" ? (
																	...
																	single_shipment_data?.shipment_data?.terminal_port
													...
													<p className="white-text text-sm">
														{moment(single_shipment_data?.shipment_data?.createdAt).format(
															"LL"
														)}
													</p>
												</div>
										...
										<div className="flex items-center mb-2 mt-3 mobile-padding">
											...
											<h3 className="text-xl black-text font-semibold">
												Shipment Created By:
												{/* {
													single_shipment_data?.shipment_data?.activity_status == "inactive" ? (
										...
										<div className="mt-5">
											{/* Tab To Keep Active Tabs The same when reloaded */}
											<NewCustomTabs
												tabs={[
													"Shipment Details",
													"Container Details",
										...
										<div className="mt-5">
											...
											{tab === "Container Details" && (
												<ContainerDetails single_shipment={single_shipment} />
											)}
											{tab === "Container Status History" && (
...
export default connect(mapStateToProps, { getSingleShipment, getContainerStatusHistory, })(ShipmentDetails);
