...
import { getRequestContacts, updateStatus } from "store/actions";
...
const RequestCallback = (props:any) => {
    const {loading, callback_contacts, updateStatus} = props;
    const [isOpen, setIsOpen] = useState(false);
    const [messageToshow, setMessageToShow] = useState({});
    const [isUpdate, setIsUpdate] = useState(false);
    const [dataToUpdate, setDataToUpdate] = useState({});
    console.log(callback_contacts?.data?.data)

    useEffect(() => {
        props.getRequestContacts();
    }, [getRequestContacts])
        ...
        <div className="mt-4">
            {
				...
				(
                    ...
                    {
                        callback_contacts?.data?.data?.status === "closed" ? (
                            <> 
                        ...
                        <>
                            <table className="desktop-only">
                                <tr className="pb-5">
                                    <th className="grey-text uppercase text-xs font-semibold text-left pl-3 pb-3">Fullname</th>
                                    <th className="grey-text mx-2 uppercase text-xs font-semibold text-left px-10 pb-3">Email Address</th>
                                    <th className="grey-text mx-2 uppercase text-xs font-semibold text-left px-10 pb-3">Time to call</th>
                                    <th className="grey-text mx-2 uppercase text-xs font-semibold text-left px-10 pb-3">Contact Type</th>
                                    <th className="grey-text uppercase text-xs font-semibold text-left pb-3 px-10">Contact Phone Number</th>
                                ...
                                {
                                    callback_contacts?.['data']?.['data'].map((data, index) => {
                                        return (
                                            <>
                                                <>
                                                            ...
                                                            <tr className="right-divider top-divider bottom-divider left-divider rounded w-full">
                                                                <td className="py-5 pl-3">
                                                                    <div className="flex items-center">
                                                                        <p className="ml-2 black-text font-semibold text-sm"style={{textTransform: 'capitalize'}}>{data?.['contact_fullname']}</p>
                                                                    </div>
                                                                </td>
                                                                <td>
                                                                    <div className="mx-10">
                                                                ...
                                                                {/* <td>
                                                                    <div className="mx-10">
                                                                        <p className="black-text font-semibold text-sm">co</p>
                                                                    </div>
                                                                </td> */}
                                                                <td>
                                                                    <p className="mx-10 black-text font-semibold text-sm"> {data?.['contact_time_to_call']}</p>
                                                                </td>
                                                                <td className="px-5">
                                                                    ...
                                                                    <p className="black-text font-semibold text-sm">
                                                                        {data?.['status'] === "open" ? (
                                                                            <Link to="#" className="bg-light-red black-text-2 text-sm py-2 px-3 red-text mx-5 text-center rounded-full">
                                                                                ...
                                                                                <Link to="#" className="bg-light-green solid-br black-text-2 text-sm py-2 px-3 green-text mx-5 text-center rounded-full">
                                                                                    Attended
                                                                                </Link>
                                                                            )
                                                                        }
...
const mapStateToProps = (state: any) => {
    const { callback_contacts, error, loading } = state.contacts;
    return { callback_contacts, error, loading };
};

export default connect(mapStateToProps, {getRequestContacts, updateStatus})(RequestCallback);
